...
from Wavelet.standardunet4 import WaveletUNetLHSeparable as WaveletUNetLHSeparable
...
parser.add_argument("--model_names", default=['WaveletUNetLHSeparable'], type=list,
...
parser.add_argument("--pth_dirs", default=['/workspace/UIU/log/ablation/attention/NUDT/WaveletUNetLHSeparable_625_best.pth.tar'], type=list) 
parser.add_argument("--dataset_dir", default='/workspace/UIU/Dataset/NUDT-SIRST', type=str, help="train_dataset_dir")
parser.add_argument("--img_norm_cfg", default=None, type=dict,
                    help="specific a img_norm_cfg, default=None (using img_norm_cfg values of each dataset)")
parser.add_argument("--save_img", default=True, type=bool, help="save image of or not")
parser.add_argument("--save_img_dir", type=str, default='/workspace/UIU/log/test_result_WaveletUNetLHSeparable',help="path of saved image")
parser.add_argument("--save_log", type=str, default='/workspace/UIU/log/NUDT-SIRST-test', help="path of saved .pth")
...
class Net(nn.Module):
    def __init__(self, model_name, mode):
        ...
        if model_name == 'PIP':
            ...
            if mode == 'train':
                self.model = WaveletUNetLHSeparable(1, 1, mode='train', deepsuper=True)
            else:
                self.model = WaveletUNetLHSeparable(1, 1, mode='test', deepsuper=True)
...
def Test():
    ...
    with torch.no_grad():
        for idx_iter, (img, gt_mask, size, img_dir) in enumerate(tbar):
            ...
            pred, _ = net.forward(img)                    #改
            # pred = net.forward(img)                    #改
